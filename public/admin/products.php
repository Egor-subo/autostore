...
<div class="col-12"><button name="save" class="btn btn-primary">Сохранить</button> <a href="/admin/products.php" class="btn btn-secondary">Очистить</a></div>
</form>
<table class="table table-sm"><tr><th>ID</th><th>Название</th><th>Тип</th><th>Категория</th><th>Цена</th><th></th></tr><?php foreach($products as $p): ?><tr><td><?= $p['id'] ?></td><td><?= e($p['title']) ?></td><td><?= e($p['type']) ?></td><td><?= e($p['category_name']) ?></td><td><?= number_format((float)$p['price'],0,',',' ') ?></td><td><a href="/admin/products.php?edit=<?= $p['id'] ?>" class="btn btn-sm btn-outline-primary">Ред.</a> <form method="post" class="d-inline"><input type="hidden" name="id" value="<?= $p['id'] ?>"><button name="delete" class="btn btn-sm btn-outline-danger" data-confirm="Удалить?">Удалить</button></form></td></tr><?php endforeach; ?></table>
